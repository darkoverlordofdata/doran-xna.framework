...
namespace Microsoft.Xna.Framework.Graphics
{
    public class VertexBuffer : GraphicsResource
    {
        ...
        public VertexBuffer.Decl(GraphicsDevice graphicsDevice, VertexDeclaration vertexDeclaration, int vertexCount, BufferUsage bufferUsage) 
        {
			this(graphicsDevice, vertexDeclaration, vertexCount, bufferUsage, false);
        }
		
		public VertexBuffer.Type(GraphicsDevice graphicsDevice, Type type, int vertexCount, BufferUsage bufferUsage) 
		{
        ...
        public void GetData1<T>(T[] data) // where T : struct
        {
        ...
        /// The combination of <paramref name="startIndex"/> and <paramref name="elementCount"/> 
        /// must be within the <paramref name="data"/> array bounds.</param>
		public void SetData2<T>(T[] data, int startIndex, int elementCount) // where T : struct
        {
            var elementSizeInBytes = Utilities.ReflectionHelpers.SizeOf<T>.Get();
        ...
        /// <typeparam name="T">Type of elements in the data array.</typeparam>
        /// <param name="data">Data array.</param>
        public void SetData1<T>(T[] data) // where T : struct
        {
            var elementSizeInBytes = Utilities.ReflectionHelpers.SizeOf<T>.Get();
        ...
        protected void SetDataInternal<T>(int offsetInBytes, T[] data, int startIndex, int elementCount, int vertexStride, SetDataOptions options) // where T : struct
        {
            ...
            var vertexByteSize = VertexCount * VertexDeclaration.VertexStride;
            if (vertexStride > vertexByteSize)
                throw new Exception.ArgumentOutOfRangeException("vertexStride - Vertex stride can not be larger than the vertex buffer size.");

            if (startIndex + elementCount > data.Length || elementCount <= 0)
                throw new Exception.ArgumentOutOfRangeException("data - The array specified in the data parameter is not the correct size for the amount of data requested.");
            if (elementCount > 1 && (elementCount * vertexStride > bufferSize))
                throw new Exception.InvalidOperationException("The vertex stride is larger than the vertex buffer.");
            if (vertexStride < elementSizeInBytes)
                throw new Exception.ArgumentOutOfRangeException("The vertex stride must be greater than or equal to the size of the specified data (" + elementSizeInBytes + ").");            
