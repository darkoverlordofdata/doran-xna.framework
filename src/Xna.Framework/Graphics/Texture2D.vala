...
namespace Microsoft.Xna.Framework.Graphics
{
    public class Texture2D : Texture
    {
        ...
        public Texture2D(GraphicsDevice graphicsDevice, int width, int height, bool mipmap, SurfaceFormat format, SurfaceType type, bool shared=false, int arraySize=1)
		{
            if (graphicsDevice == null)
                throw new Exception.ArgumentNullException("graphicsDevice - FrameworkResources.ResourceCreationWhenDeviceIsNull");
            if (width <= 0)
                throw new Exception.ArgumentOutOfRangeException("width Texture width must be greater than zero");
            if (height <= 0)
                throw new Exception.ArgumentOutOfRangeException("height Texture height must be greater than zero");
            if (arraySize > 1 && !graphicsDevice.GraphicsCapabilities.SupportsTextureArrays)
                throw new Exception.ArgumentException("Texture arrays are not supported on this graphics device arraySize");
...
//         {
//             var textureBounds = new Rectangle(0, 0, int.max(width >> level, 1), int.max(height >> level, 1));
//             checkedRect = rect ?? textureBounds;
//             if (level < 0 || level >= LevelCount)
//                 throw new Exception.ArgumentException("level must be smaller than the number of levels in this texture. level");
//             if (arraySlice > 0 && !GraphicsDevice.GraphicsCapabilities.SupportsTextureArrays)
//                 throw new Exception.ArgumentException("Texture arrays are not supported on this graphics device arraySlice");
//             if (arraySlice < 0 || arraySlice >= ArraySize)
//                 throw new Exception.ArgumentException("arraySlice must be smaller than the ArraySize of this texture and larger than 0. arraySlice");
//             if (!textureBounds.Contains(checkedRect) || checkedRect.Width <= 0 || checkedRect.Height <= 0)
//                 throw new Exception.ArgumentException("Rectangle must be inside the texture bounds rect");
//             if (data == null)
//                 throw new Exception.ArgumentNullException("data");
//             var tSize = sizeof(T); //ReflectionHelpers.SizeOf<T>.Get();
//             var fSize = Format.GetSize();
//             if (tSize > fSize || fSize % tSize != 0)
//                 throw new Exception.ArgumentException("Type T is of an invalid size for the format of this texture. T");
//             if (startIndex < 0 || startIndex >= data.length)
//                 throw new Exception.ArgumentException("startIndex must be at least zero and smaller than data.length. startIndex");
//             if (data.length < startIndex + elementCount)
//                 throw new Exception.ArgumentException("The data array is too small.");
...
//                 throw new Exception.ArgumentException("elementCount is not the right size, " +
//                                             "elementCount * sizeof(T) is %d, but data size is %d."
//                                             .printf(elementCount * tSize, dataByteSize));
//         }
	}
}
